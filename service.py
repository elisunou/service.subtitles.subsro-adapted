...
def download_subtitle(sub_id, download_link=None):
    ...
    try:
        ...
        def pick_and_read_xbmcvfs(archive_path, vf):
            """
            Extrage subtitrare din arhivă (ZIP sau RAR) via xbmcvfs.
            Kodi suportă nativ ambele formate prin vfs://arhiva/fisier.
            Nu necesită niciun program extern — funcționează pe Windows,
            Linux, Android, etc.
            Returnează (bytes_content, filename) sau (None, None).
            """
            # xbmcvfs listează conținutul arhivei via path special:
            # zip://path%2Fla%2Farhiva.zip  sau  rar://path%2Fla%2Farhiva.rar
            ext = os.path.splitext(archive_path)[1].lower()

            # Detectare tip din magic bytes (mai fiabil decât extensia)
            try:
            ...
            log(f"Tip arhiva: {ext.upper()} (via magic bytes)")
            ...
            if ext == '.zip':
                ...
                log(f"Tip arhiva necunoscut: {ext}", xbmc.LOGWARNING)
                return None, None

            # Construiește URL-ul VFS Kodi: scheme://path_encoded/
            encoded = urllib.parse.quote(archive_path, safe='')
            vfs_root = f"{vfs_scheme}://{encoded}/"
            log(f"VFS root: {vfs_root}")

            # Listează fișierele din arhivă
            try:
                dirs, files = xbmcvfs.listdir(vfs_root)
            except Exception as e:
                log(f"xbmcvfs.listdir eroare: {e}", xbmc.LOGERROR)
                return None, None

            # Filtrează subtitrările
            srts = sorted([f for f in files if f.lower().endswith(SUB_EXTS)])
            log(f"Fisiere in arhiva: {files} | subtitrari: {srts}")

            if not srts:
                return None, None

            # Alege subtitrarea conform setării multi_episode_handling
            multi = ADDON.getSetting('multi_episode_handling')
            if len(srts) == 1:
                chosen = srts[0]
            elif multi == '0':  # Manual
                sel = xbmcgui.Dialog().select(
                    "Alege subtitrarea:", srts
                )
                if sel == -1:
                    return None, None
                chosen = srts[sel]
                ...
                chosen = max(srts,
                             key=lambda n: calculate_match_score(n, vf)[0])
                log(f"Auto-selectat: {chosen}")

            # Citește fișierul din arhivă via xbmcvfs
            vfs_file = vfs_root + chosen
            log(f"Citesc din VFS: {vfs_file}")
            try:
                fh = xbmcvfs.File(vfs_file)
                raw_bytes = bytes(fh.readBytes())
                fh.close()
                if raw_bytes:
                    return raw_bytes, chosen
                log("xbmcvfs.File: continut gol", xbmc.LOGWARNING)
                return None, None
            except Exception as e:
                log(f"xbmcvfs.File eroare: {e}", xbmc.LOGERROR)
                return None, None

        chosen_name    = None
        chosen_content = None

        # ── Extragere via xbmcvfs (ZIP + RAR nativ Kodi) ────────────────────
        chosen_content, chosen_name = pick_and_read_xbmcvfs(archive, player.getPlayingFile())

        if chosen_content is None:
            xbmcgui.Dialog().notification(
                "Subs.ro", "Nu s-au gasit subtitrari in arhiva.",

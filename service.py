...
def download_subtitle(sub_id, download_link=None):
    ...
    try:
        ...
        is_rar = raw[:4] == b'Rar!'
        log(f"Tip arhiva: {'ZIP' if is_zip else 'RAR' if is_rar else 'necunoscut'} "
            f"(magic: {raw[:4].hex()})")

        def decode_subtitle(raw_bytes):
            """Decodare automata encoding — utf-8, iso-8859-2, windows-1250, latin1."""
        ...
        def pick_from_list(names, vf):
            """Alege subtitrarea din lista de nume."""
            ...
            if multi == '0':
                ...
                best = max(srts, key=lambda n: calculate_match_score(os.path.basename(n), vf)[0])
        ...
        # ── Extragere ZIP — zipfile Python built-in (garantat Kodi 21) ───────
        if is_zip:
            try:
                with zipfile.ZipFile(archive, 'r') as z:
                    chosen_name = pick_from_list(z.namelist(), player.getPlayingFile())
        ...
        # ── Extragere RAR — via addon vfs.rar (Kodi 21 repo oficial) ─────────
        # vfs.rar se instaleaza din: Kodi -> Settings -> Add-ons -> Install from repo
        # Dupa instalare expune rar:// in xbmcvfs — fara programe externe
        if is_rar:
            rar_ok = False

            # Metoda 1: vfs.rar addon via xbmcvfs (recomandat pe Kodi 21)
            try:
                ...
                if files:
                    chosen_name = pick_from_list(files, player.getPlayingFile())
                    if chosen_name:
                ...
                log(f"vfs.rar indisponibil: {e}", xbmc.LOGWARNING)

            # Metoda 2: subprocess WinRAR / 7-Zip / unrar
            if not rar_ok:
                log("Incerc extragere RAR via subprocess...", xbmc.LOGWARNING)
                chosen_content, chosen_name = _extract_rar_subprocess(
                    archive, tmp_path, player.getPlayingFile()
                )
                if chosen_content:
                    log(f"RAR extras via subprocess: {chosen_name}")
                    rar_ok = True

            # Nimic nu a mers — afisam instructiuni clare
            if not rar_ok:
                xbmcgui.Dialog().ok(
                    "Subs.ro - Arhiva RAR",
                    "Nu s-a putut deschide arhiva RAR.\n\n"
                    "Solutia recomandata pentru Kodi 21:\n"
                    "Instaleaza addon-ul [B]vfs.rar[/B] din:\n"
                    "Settings -> Add-ons -> Install from repository\n"
                    "-> Kodi Add-on repository -> Addons\n"
                    "-> VFS Add-ons -> RAR filesystem support"
                )
                return

...
def download_subtitle(sub_id, download_link=None):
    ...
    archive  = os.path.join(tmp_path, f"subsro_{sub_id_int}.zip")
    ...
    try:
        ...
        with zipfile.ZipFile(archive, 'r') as z:
            srts = sorted([n for n in z.namelist()
                           if n.lower().endswith(('.srt', '.ass', '.ssa'))])
            if not srts:
                xbmcgui.Dialog().notification(
                    "Subs.ro", "Arhiva nu conține subtitrări (.srt/.ass)",
            ...
            if len(srts) > 1:
                if multi == '0':  # Manual
                    ...
                    log(f"Auto-selectat: {os.path.basename(chosen)}")
            else:
                chosen = srts[0]

            # Decodare cu detectare automată encoding
            content   = z.read(chosen)
            ...
            if text is None:
                ...
                log("Encoding fallback: latin1 cu replace", xbmc.LOGWARNING)
        ...
        deadline = time.time() + 6  # max 6 secunde
